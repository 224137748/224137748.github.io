...
Vue.js 利用 createElement 方法创建 VNode，它的源码如下：
...
回到上面省略的 `createElement` 函数，规范化 `children` 后，接下来会去创建一个 VNode 的实例：
...
if (typeof tag === "string") {
  ...
  if (config.isReservedTag(tag)) {
    // platform built-in elements 平台相关的标签
    vnode = new VNode(
